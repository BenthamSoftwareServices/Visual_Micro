...
/*
  Eyes project at: https://www.youtube.com/watch?v=pmCc7z_Mi8I&t=1000s
  For better use of safe GPIO pins on the ESP32 see: https://dronebotworkshop.com/gc9a01/
  Eye types can be edited in config.h


  I had a problem with uploading the code, which I eventually traced to the display's DC pin being connected to GPIO2 on the ESP32; a common problem apparently.
  I could upload the code with the DC disconnected and then by re-connecting DC and resetting the ESP32 the "eye" appeared on the display - not ideal.
  I then edited User_Setup.h file as follows to move the display pins to safer GPIOs.
	 ...
	 On lines 223 through 228 comment out all the SPI definitions for the GC9A01.
	 Add the following lines:
	 On line 230 set MOSI to 23 //DIN
	 On line 231 set SCLK to 18 //CLK
	 On line 232 set CS to 22   //CS
	 On line 233 set DC to 16   //DC
	 On line 234 set RST to 4   //RST

  In User_Setup_Select.h make sure that line 85 is commented out i.e. //#include <User_Setups/Setup46_GC9A01_ESP32.h> // Setup file for ESP32 and GC9A01 SPI bus TFT  240x240
  Otherwise the changes to User_Setup.h will be ignored and the display won't work.

  I think another approach could be to edit Setup46_GC9A01_ESP32.h and change the GPIOs to the safe numbers shown above and then uncomment User_Setup_Select.h line 80
  i.e. #include <User_Setups/Setup46_GC9A01_ESP32.h>

  Anyone using PlatformIO and an ESP-PROG JTAG programmer probably won't see this potential problem.
  ...
  chip selects and x offsets can be defined in the header file "config.h".
...
#include <SPI.h>
#include <TFT_eSPI.h>
//#include "src/library/TFT_eSPI.h"
//#include "E:\Users\Steven\Documents\GitHub\Visual_Micro\ESP32\ESP32LCDRound240x240Eyes\arduino-solution\libraries\TFT_eSPI.h"
